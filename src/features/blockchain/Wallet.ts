...
export default class Wallet {
    ...
    constructor(data: { [key: string] : WalletData }) {
        ...
        this.NFTMap.set('ETH', new NFT(ethData.Network, ethData.TokenAddress, ethData.Contract721Address, ethData.Contract1155Address, ethData.ContractMarketAddress));

        var bscKey: string = gConfig.isProd? 'addrBscProd' : 'addrBscDev';
        var bscData: WalletData = data[bscKey];
        this.NFTMap.set('BSC', new NFT(bscData.Network, bscData.TokenAddress, bscData.Contract721Address, bscData.Contract1155Address, bscData.ContractMarketAddress));

        var polKey: string = gConfig.isProd? 'addrPolygonProd' : 'addrPolygonDev';
        var polData: WalletData = data[polKey];
        this.NFTMap.set('POL', new NFT(polData.Network, polData.TokenAddress, polData.Contract721Address, polData.Contract1155Address, polData.ContractMarketAddress));

        var bsc3rdKey: string = gConfig.isProd? 'addr3rdLandBscProd' : 'addr3rdLandBscDev';
        var bsc3rdData: WalletData = data[bsc3rdKey];
        this.NFTMap.set('BSC3', new NFT(bsc3rdData.Network, bsc3rdData.TokenAddress, bsc3rdData.Contract721Address, bsc3rdData.Contract1155Address, bsc3rdData.ContractMarketAddress));

        var pol3rdKey: string = gConfig.isProd? 'addr3rdLandPolProd' : 'addr3rdLandPolDev';
        var pol3rdData: WalletData = data[pol3rdKey];
        this.NFTMap.set('POL3', new NFT(pol3rdData.Network, pol3rdData.TokenAddress, pol3rdData.Contract721Address, pol3rdData.Contract1155Address, pol3rdData.ContractMarketAddress));
