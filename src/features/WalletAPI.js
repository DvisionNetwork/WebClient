...
export default class walletAPI {
	...
	async checkMetamask(provider = null) {
		var network = 'NONE'
		var addr = this.NFTWallet.getAddr('ETH')
		var bscAddr = this.NFTWallet.getAddr('BSC')
		var polygonAddr = this.NFTWallet.getAddr('POL')
	...
	async getDviBalance(account, provider = null, network, callback) {
		...
		this.lv_provider = new ethers.providers.Web3Provider(
	...
	getMarketAddr(network) {
		var addr = this.NFTWallet.getAddr(network, "-1")
	...
	getContract(type, network, nft, loginBy, market_index) {
		const addr = this.NFTWallet.getAddr(network, market_index)
		const networkRPC = window.localStorage.getItem('networkRPC')
		...
		if (market_index != '-1') {
			...
			if (loginBy === FORTMATIC || loginBy === BITSKI) {
				...
				contract = new ethers.Contract(address, abi, this.lv_signer)
			}
		} else {
			if (type == 'Approval') {
				if (loginBy === FORTMATIC || loginBy === BITSKI) {
	...
	getContractNew(type, network, nft, loginBy) {
		const addr = (network == 'BSC') ? this.NFTWallet.getAddr('BSC3rd') : this.NFTWallet.getAddr('POL3rd');
	...
	async ContractDvi(J) {
		...
		const loginBy = window.localStorage.getItem('loginBy')
			// 04.25 Market
			const contAddr = this.NFTWallet.getContAddr(J.category, J.network)
		if (!contAddr) {
			J.callback({
				res_code: 401,
		...
		console.log(this.lv_provider, this.lv_signer, 'provider, signer')
		...
		if(J.market_index != "-1") {
			const marketAddr = NFTWallet.getContAddr("market", J.network)
		...
		if (rv != 'NONE') {
			// 04.25 Market
			const approveAddr = J.market_index != "-1" ? this.NFTWallet.getContAddr("market", J.network) : contAddr;
			try {
				J.fToast(
					`${J.type} ${
	...
	async ContractDviNew(J) {
		...
		const loginBy = window.localStorage.getItem('loginBy')
		const contAddr = (J.network == 'BSC') ? this.NFTWallet.getAddr('BSC3rd').ContractMarketAddress : this.NFTWallet.getAddr('POL3rd').ContractMarketAddress;
	...
	async getOwnAmount(J) {
		...
		if (J.category == '1155') {
			...
			var contAddr = this.NFTWallet.getContAddr(J.category, J.network)
